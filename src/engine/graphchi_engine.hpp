...
#include "engine/new_scheduler.hpp"
    ...
    protected:
        ...
        virtual void initialize_scheduler() {
            if (use_selective_scheduling) {
                ...
                scheduler = new new_scheduler((int) num_vertices());
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    public:
        ...
        void run(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int _niters) {
			...
			*/
            ...
            for(iter=0; iter < niters; iter++) {
                ...
                if (use_selective_scheduling) {
                    if (scheduler != NULL) {
                        if (!scheduler->has_new_tasks) {
							...
							////////////////////////////
							//modified by mzj
							chicontext.set_last_iteration(iter);
                            break;
                        }
                        scheduler->has_new_tasks = false; // Kind of misleading since scheduler may still have tasks - but no new tasks.
                    }
                ...
                for(int interval_idx=0; interval_idx < nshards; ++interval_idx) {
                    ...
                    while (sub_interval_st <= interval_en) {
						...
						/////////////////////////
						chicontext.interval_st = sub_interval_st;
						chicontext.interval_en = sub_interval_en;
            ...
            //m.set("niters", niters);
			//modified by mzj
            m.set("niters", chicontext.last_iteration + 1);
    ...
    public:
        ...
        vertex_data_store<VertexDataType> * get_vertex_data_handle(){
				return vertex_data_handler;
		}
