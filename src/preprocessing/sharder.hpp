    ...
    protected:
        ...
        virtual void determine_number_of_shards(std::string nshards_string) {
            ...
            if (nshards_string.find("auto") != std::string::npos || nshards_string == "0") {
				...
				if(isdigit(nshards_string[0]) || nshards_string[0] == '+') {
				...
				}else{//enable user to specify interval before partitioning. Added by mzj
					...
					while(fgets(buffer, 1024, fpi) != NULL){
						...
						int istart = atoi(start);
						prange.push_back(istart);	
						char* end = strtok(NULL, delim);
						int iend = atoi(end);
						prange.push_back(iend);	
						std::cout<<"push "<<istart<<"-->"<<iend<<std::endl;
					}
					nshards = prange.size() / 2;
					std::cout<<"prange  nshards="<<nshards<<std::endl;
				}
            }
            assert(nshards > 0);
	...
	//	vid_t intervalend;
		...
		*/	
			...
			if(prange.size() == 0){
				...
				if(val.dst > prange[2*gindex + 1]){
					/*
					std::cout<<"++++++++++++++++++++++++++++++++interval: "<<gindex<<"\t endvid: "<<prange[2*gindex+1]
						<<"\tval.dst: "<<val.dst<<std::endl;
					*/
					/*important bug fix: 
						when the end point vid of interval i does not have in edges(indegree is zero), 
						it would be skipped by the prevvid
					*/
					prevvid = prange[2*gindex + 1];
					createnextshard();
					gindex++;
				}				
			}
        ...
        virtual void write_shards() {
           ...
           // sinkbuffer = (edge_with_value<EdgeDataType> *) calloc(shard_capacity, sizeof(edge_with_value<EdgeDataType>));
			...
			if(prange.size() == 0){
				...
				for(int i=0; i<(int)prange.size(); i+=2){
					intervalarray.push_back(prange[i]);		
				}
			}*/
