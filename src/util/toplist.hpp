...
////////////////////////////////////////////////////////////////////////////////////
		...
		template <typename VertexDataType>
			bool vertex_degree_greater(const vertex_value<VertexDataType> &a, const vertex_value<VertexDataType> &b) {
				return a.value.get_degree() > b.value.get_degree();
			}

		template <typename VertexDataType>
			//std::set<vid_t> get_top_degree_vertices(std::string basefilename, int ntop, vid_t from=0, vid_t to=0) {
			std::set<vid_t> get_top_degree_vertices(std::string basefilename, int ntop, int* maxdeg = NULL, vid_t from=0, vid_t to=0) {
				typedef vertex_value<VertexDataType> vv_t;
				...
				while(st <= numvertices - 1) {
					...
					for(int j=0; j < nt; j++) {
						VertexDataType& val = *vertexdata->vertex_data_ptr(j + st);
						if(val.is_active()){
							if (count == 0 || (val.get_degree() > minima.get_degree())) {
								buffer_idxs[k] = vv_t((vid_t)idx + from, val);
								k++;
							}
						}
					...
					/* Sort buffer-idxs */
					quickSort(buffer_idxs, nt, vertex_degree_greater<VertexDataType>);
					...
					if (count == 0) {
						...
						merge<vv_t>(topbuf, ntop, buffer_idxs, std::min(ntop, nt), mergearr,  vertex_degree_greater<VertexDataType>);
						memcpy(topbuf, mergearr, ntop * sizeof(vv_t));
					}
				...
				std::set<vid_t> ret;
				for(int i=0; i < ntop; i++) {
					//ret.push_back(topbuf[i].vertex);
					ret.insert(topbuf[i].vertex);
				}
				//////////////////	
				if(maxdeg != NULL)	
					*maxdeg = topbuf[0].value.get_degree();
				/////////////////
				free(buffer_idxs);
				free(mergearr);
				free(topbuf);
