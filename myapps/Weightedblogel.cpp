...
int main(int argc, const char ** argv) {
    ...
    metrics m("Weightedblogel-partitioning");
	...
	orig_filename += ".txt";
	pfilename = orig_filename + ".blogel.vmap";	
	initIdMap();
    ...
    //engine.set_modifies_inedges(false); // Improves I/O performance.
   	fp_list = fopen((filename+".blogel").c_str(), "w+");	 
	...
	FILE* srcfp_interval = fopen((orig_filename+".blogel.interval").c_str(), "r");
	assert(srcfp_interval != NULL);
	FILE* dstfp_interval = fopen((filename+".blogel.interval").c_str(), "w+");
	assert(dstfp_interval != NULL);
	char tmp[1024];
	std::cout<<"output file is "<<filename+".blogel.interval"<<std::endl;
