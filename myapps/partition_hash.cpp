...
struct ReMapProgram: public GraphChiProgram<VertexDataType, EdgeDataType> {
	...
	void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
		if (gcontext.iteration == 0){
				...
				//id_th = vertex.id();	
				vid_t row = vertex.id() / Parts; 
				//vid_t new_id = row * Parts + prefix_sum[vertex.id() % Parts];	
				vid_t new_id = row + prefix_sum[vertex.id() % Parts];	
...
void prefix_sum_calculate(int shards, int nvertices){
	...
	std::cout<<shards<<" interval range has write to file"<<std::endl;
	/*
	for(int i=0; i<shards; i++){
...
int main(int argc, const char ** argv) {
    ...
    Parts            	 = get_option_int("num_par", 5);//number of partitions to be created
    /* Process input file - if not already preprocessed */
    int nshards            = (int) convert_if_notexists<EdgeDataType>(filename, get_option_string("nshards", "auto"));
    /* Run */
	efout = fopen((filename+".hash").c_str(), "w+");	
	vfout = fopen((filename+".hash.vmap").c_str(), "w+");
	fp_interval = fopen((filename+".hash.interval").c_str(), "w+");
	assert(vfout != NULL && efout != NULL && fp_interval != NULL);
	...
	prefix_sum_calculate(Parts, num_vertices);

	ReMapProgram remapprogram;				
	engine2.set_exec_threads(1);
	engine2.run(remapprogram, niters);
	fclose(vfout);
	fclose(efout);
	fclose(fp_interval);
	std::cout<<"hash based partitioning has been remapped"<<std::endl;
