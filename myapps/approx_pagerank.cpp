...
struct PagerankProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &v, graphchi_context &ginfo) {
        ...
        if (ginfo.iteration == 0) {
			...
			/*		
			//if(ginfo.iteration > 3 && pagerank > old_value){
			if(ginfo.iteration > 3 && pagerank > old_value){
				std::cout<<"vid="<<v.id()<<" pagerank="<<pagerank<<" old_rank="<<old_value<<" indeg="<<v.num_inedges()<<" outdeg="<<v.num_outedges()<<std::endl;
				assert(false);
			}
			*/	
            ...
            ginfo.log_change(std::abs(pagerank - old_value));
