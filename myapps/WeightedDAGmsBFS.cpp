...
int main(int argc, const char ** argv) {
    ...
    metrics m("WeightedDAGmsBFS-partitioning");
	...
	orig_filename += ".txt";
	pfilename = orig_filename + ".dag.vmap";	
	initIdMap();
    ...
    //engine.set_modifies_inedges(false); // Improves I/O performance.
   	fp_list = fopen((filename+".dag").c_str(), "w+");	 
	...
	FILE* srcfp_interval = fopen((orig_filename+".dag.interval").c_str(), "r");
	assert(srcfp_interval != NULL);
	FILE* dstfp_interval = fopen((filename+".dag.interval").c_str(), "w+");
	assert(dstfp_interval != NULL);
	char tmp[1024];
	std::cout<<"output file is "<<filename+".dag.interval"<<std::endl;
